...
public class DistributedLock implements Watcher {
    ...
    public DistributedLock(String lockName) throws IOException, KeeperException, InterruptedException {
        ...
        client = new ZooKeeperClient(zooKeeperUrl, 5000,
                this);
        startFlag.await();
    ...
    private void createRootNode() throws InterruptedException, UnsupportedEncodingException,
            KeeperException {
        lockPath = client.createNode(lockPath, false, CreateMode.PERSISTENT);
        System.out.println("Root node created at " +
                lockPath);
    }
    private void createChildNode() throws
            InterruptedException, UnsupportedEncodingException,
            KeeperException {
        childPath = client.createNode(lockPath +
                        lockProcessPath, false,
                CreateMode.EPHEMERAL_SEQUENTIAL);
        System.out.println("Child node created at " +
                childPath);
    }
